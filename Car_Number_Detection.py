...
def carplate_extract(carplate_img_rgb):
    try:
        ...
        for x, y, w, h in carplate_rects:
            carplate_img = carplate_img_rgb[y:y + h,
                           x:x + w]  # Adjusted to extract specific region of interest i.e. car license plate
    ...
    try:
        if ("IND" or "ND" or "NND" or "TND") in (response_textract["Blocks"][1]["Text"]):
            ...
            if ("NND" or "TND" or "ND") in p:
                return ((((re.sub("[^0-9A-Z]", "", response_textract["Blocks"][1]["Text"]).replace("NND", "")).replace(
                    "TND", "")).replace("IND", "")).replace("ND", ""))
            else:
                return (p)
...
def rto(final_detected_number_plate):
    try:
        ...
        # print(r.content)
        n = xmltodict.parse(
            r.content)  # xmltodict. parse() method takes an XML file as input and changes it to Ordered Dictionary.
        # print(n)
        k = json.dumps(n)
        # print(k)
        df = json.loads(k)
        ...
        s = "\nVehicle Registration Number : " + vehicle_reg_no + "\nOwner name : " + str(
            ...
            p['CarModel']['CurrentTextValue']) + "\n" + "Fuel Type : " + str(
            p['FuelType']['CurrentTextValue']) + "\n" + "Registration Year : " + str(
            p['RegistrationYear']) + "\n" + "Insurance : " + str(p['Insurance']) + "\n" + "Vehicle ID : " + str(
            p['VechileIdentificationNumber']) + "\n" + "Engine No. : " + str(
